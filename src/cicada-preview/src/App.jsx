...
export default function App() {
  ...
  const nav = (next) => { setView(next); window.scrollTo({ top: 0, left: 0, behavior: 'instant' }); };
  ...
  const handleScanError = () => {
    setScanError('We could not complete this check. The analysis service may be unavailable; please try again later.');
    setView('checker');
  };
  ...
  return (
    <div className="min-h-screen flex flex-col">
        ...
        <>
          {scanError && (
            <div className="max-w-3xl mx-auto px-6 pt-6 -mb-4">
              <p role="alert" className="text-[13px] font-medium" style={{ color: 'var(--critical)' }}>{scanError}</p>
            </div>
          )}
      ...
      {view === 'clinician' && <Auth mode="clinician" onAuthenticated={() => { window.location.href = `${window.location.protocol}//${window.location.hostname}:5000/clinical`; }} onSkip={() => nav('checker')} />}
