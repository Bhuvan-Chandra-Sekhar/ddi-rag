    ...
    <header className="site-header">
      <div className="header-inner">
        <button onClick={() => onNav('home')} className="brand flex items-center gap-3" aria-label="MedTrust Connect home">
          ...
          <span className="brand-name whitespace-nowrap">medtrust<span className="brand-period">.</span></span>
        </button>
        <nav className="site-nav label-tag" aria-label="Main navigation">
          <button onClick={() => onNav('home')} aria-current={view === 'home' ? 'page' : undefined}>Overview</button>
          <button onClick={() => onNav('checker')} aria-current={view === 'checker' ? 'page' : undefined}>Quick check</button>
