...
function OverallBanner({ overallSeverity, seeADoctor, hasUnresolvedMed, coverageOutlier }) {
  if (seeADoctor) {
    return (
  ...
  if (!overallSeverity && (hasUnresolvedMed || coverageOutlier)) {
    return (
      <Banner color="var(--caution)" bg="rgba(255,184,41,0.16)">
        <p className="font-display text-[27px] font-normal mb-3">We could not confirm the full medication match</p>
        <p className="text-[13px] leading-relaxed" style={{ color: 'var(--ink-soft)' }}>
          One or more entries may have matched approximately or expanded into many possible ingredients.
          Check each medication detail below and try the exact name on its label. No findings here
          should be interpreted as confirmation that the combination is safe.
        </p>
      </Banner>
    );
  }
...
export default function Results({ data, checkingCount, draft, onNewCheck, onCheckAnother }) {
  ...
  const coverageOutlier = (coverage?.pairs_checked ?? 0) > Math.max(20, Math.pow(resolvedMedications.length * 10, 2));
  ...
  const [visiblePairsCount, setVisiblePairsCount] = useState(20);
  ...
  return (
    <main className="flex-1 w-full max-w-3xl mx-auto px-6 py-16 sm:py-24">
      <div className="reveal space-y-9">
        ...
        <OverallBanner overallSeverity={overall_severity} seeADoctor={see_a_doctor} hasUnresolvedMed={hasUnresolvedMed} coverageOutlier={coverageOutlier} />
              ...
              {coverage.unmatched_pairs?.length > 0 && (
                <details className="mt-4 text-[14px]" style={{ color: 'var(--ink-soft)' }}>
                  ...
                  <ul className="mt-3 space-y-2">
                    {coverage.unmatched_pairs.slice(0, visiblePairsCount).map((pair, index) => <li key={index}>{pair.ingredient_a} + {pair.ingredient_b}</li>)}
                  </ul>
                  {coverage.unmatched_pairs.length > visiblePairsCount && (
                    <button type="button" className="mt-4 underline underline-link" onClick={() => setVisiblePairsCount((count) => count + 20)}>
                      Show 20 more pairs
                    </button>
                  )}
                </details>
